...
| `EMAIL_PROVIDER_NAME`               | `the configured mailbox provider`                                | Destination mailbox provider                                       |
...
| `EMAIL_PROVIDER_MODE`               | `mailbox-provider`, `consumer-gmail`, or `google-workspace`      | Selects provider-specific mailbox wording                          |
...
| the configured mailbox provider | Authenticated SMTP delivery and mailbox storage for support messages  | Keep the provider, retention period, and applicable processing records aligned with the approved production deployment.                                                                                                                                                                                                                                                                                                                                                       |
...
| ----------------------------------- | ------------------------------------------- | -------------------------------------------------- |
| `CONTACT_SMTP_HOST`                 | deployment-only SMTP endpoint               | TLS SMTP endpoint                                  |
| `CONTACT_SMTP_PORT`                 | `465`                                       | SMTP port                                          |
| `CONTACT_SMTP_SECURE`               | `true`                                      | Use implicit TLS                                   |
| `CONTACT_SMTP_USERNAME`             | deployment-only mailbox account             | Authenticated SMTP account                         |
...
| `CONTACT_FROM_ADDRESS`              | deployment-only mailbox account             | DMARC-aligned envelope/header sender               |
| `CONTACT_TO_ADDRESS`                | approved support address                    | Support mailbox                                    |
...
- Verify that the configured mailbox provider continues to apply the documented operational deletion period. If the provider or forwarding arrangement changes, update `EMAIL_PROVIDER_MODE` and this notice before deployment.
