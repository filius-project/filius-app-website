...
| `EMAIL_PROVIDER_NAME`               | `mailbox-provider GmbH`                                                    | Destination mailbox provider                                       |
...
| `EMAIL_PROVIDER_MODE`               | `mailbox-provider-mailbox`, `consumer-gmail`, or `google-workspace`        | Selects provider-specific mailbox wording                          |
...
| ----------------------------------- | ------------------------------------------- | -------------------------------------------------- |
| `CONTACT_SMTP_HOST`                 | `provider-smtp.example.invalid`                         | TLS SMTP endpoint                                  |
| `CONTACT_SMTP_PORT`                 | `465`                                       | SMTP port                                          |
| `CONTACT_SMTP_SECURE`               | `true`                                      | Use implicit TLS                                   |
| `CONTACT_SMTP_USERNAME`             | `deployment-mailbox@example.invalid`                        | Authenticated SMTP account                         |
...
| `CONTACT_FROM_ADDRESS`              | `deployment-mailbox@example.invalid`                        | DMARC-aligned envelope/header sender               |
...
- Verify that the active the configured mailbox provider mailbox continues to apply the documented 180-day operational deletion period. If mail is later forwarded to personal Gmail or Google Workspace, change `EMAIL_PROVIDER_MODE` and update this notice before deployment.
